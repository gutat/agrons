...
onMounted(async () => {
  try {
    const { data, error } = await supabase.from('home_section').select('*').maybeSingle()
    if (error) {
  ...
  <div>
    ...
    <form v-else @submit.prevent="save" style="width: 100%; display: flex; flex-direction: column; gap: 20px;">
      ...
      <div class="adm-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
        <div style="background: white; border-radius: 16px; border: 1px solid rgba(24,28,29,0.08); padding: 24px;">
          <TranslatedField v-model:en="form.tagline" v-model:translations="form.translations" field="tagline" label="Tagline" />
        </div>
        <div style="background: white; border-radius: 16px; border: 1px solid rgba(24,28,29,0.08); padding: 24px;">
          <TranslatedField v-model:en="form.subtitle" v-model:translations="form.translations" field="subtitle" label="Subtitle" />
        </div>
      </div>

      <!-- Description -->
      <div style="background: white; border-radius: 16px; border: 1px solid rgba(24,28,29,0.08); padding: 24px;">
        <TranslatedField v-model:en="form.description" v-model:translations="form.translations" field="description" label="Description" textarea :rows="3" />
      </div>
